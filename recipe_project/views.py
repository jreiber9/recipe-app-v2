    ...
    if request.method == 'POST':       
        ...
        if form.is_valid():                                
            ...
            #then use pre-defined Django function to login
                login(request, user)                
                return redirect('recipes:records') #& send the user to desired page
...
def logout_view(request):                                  
    ...
    return render(request, 'auth/logout_success.html') #after logging out go to login form (or whichever page you want)